...
num_questions = 10  # number of TriviaQA questions to use for benchmarking
...
# using rc.nocontext variant - lightweight, just questions and answers
...
profile_name = f"profile_{model_name.replace('/', '_')}_trivia"
...
# extract questions from TriviaQA dataset
# select first num_questions from validation set
selected_samples = dataset.select(range(num_questions))

print(f"\nSelected {num_questions} questions from TriviaQA")
print("="*60)
print("DATASET SAMPLE STRUCTURE (First item):")
print("="*60)
print(json.dumps(selected_samples[0], indent=2, default=str))
print("="*60)

prompts = [item['question'] for item in selected_samples]
ground_truth_answers = [item['answer'] for item in selected_samples]

print(f"\nExtracted {len(prompts)} questions")
print(f"Example question: {prompts[0]}")
print(f"Example answer: {ground_truth_answers[0]['value']}")
print(f"Answer aliases: {ground_truth_answers[0]['aliases'][:3]}...")  # Show first 3 aliases
...
print("\n" + "="*60)
print("INFERENCE RESULTS (First 3 samples)")
print("="*60)
for i in range(min(3, len(outputs))):
    output = outputs[i]
    question = output.prompt
    generated_text = output.outputs[0].text
    correct_answer = ground_truth_answers[i]['value']
...
print(f"Dataset: TriviaQA (rc.nocontext, validation)")
print(f"Questions: {num_questions}")
print(f"Dataset Load Time: {dataset_load_time:.4f}s")
...
output_file = f"benchmark_{model_name.replace('/', '_')}_trivia_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
with open(output_file, 'w') as f:
    json.dump(benchmark, f, indent=2)

print(f"\nBenchmark saved to: {output_file}")
print(f"\nTo profile this model with Nsight Systems, run:")
print(f"nsys profile -o {profile_name} --force-overwrite true python3 vllm_trivia_demo.py")
print("done")
